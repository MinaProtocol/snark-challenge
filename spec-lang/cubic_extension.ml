...
arithmetic, which we'll need for multi-exponentiation.|md}
  ...
  ; sec ~title:"Definitions and review"
      [ md
          {md|Let's review what exactly a field extension is. The actual operations
...
has $q^3$ elements.|md}
          (q MNT6) pages.quadratic_extension ]
  ; sec ~title:"The problem"
      [ md
          {md|In code, you can think of an element of $\mathbb{F}_{q^3}$ as a tuple
`(a0, a1, a2)` where
...
let interface : _ Problem.Interface.t =
  ...
  in
  let%map _x0 = !Input "x" arr
  and _x1 = !Input "y" arr
  and _output = !Output "z" (Type.field (Literal fqe)) in
  ksprintf Markdown.of_string
    !{md|%s
    
The output `z` should have `z[i] = x[i] * y[i]`
...
The Toom-Cook method should be faster, but it's slightly more complicated to implement.|md}
      ] ]
